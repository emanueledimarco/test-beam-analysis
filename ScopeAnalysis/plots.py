...
class PlotMaker:
    ...
    def secondaryVarPlot(self,var,range):
        ...
        for run,xpos in _xpos.iteritems():
            ...
            point +=1
        csize=600
        c1 = ROOT.TCanvas("dist_canvas","dist",csize,csize)
        c1.SetWindowSize(csize + (csize - c1.GetWw()), csize + (csize - c1.GetWh()));
        ...
        def removeOffset(gr,a,b):
            print a, "   ",b
            xp=[x+b/a for x in gr.GetX()]
            grshift=ROOT.TGraphErrors(len(xp),array('f',xp),array('f',gr.GetY()),array('f',gr.GetEX()),array('f',gr.GetEY()))
            return grshift
        grp=removeOffset(gr,vel,pol.GetParameter(0))
        customROOTstyle()
        grp.SetTitle("")
        ...
        grp.Draw("APE")
        grp.Fit("pol1")
        grp.GetXaxis().SetLimits(0,25)
        grp.GetYaxis().SetRangeUser(0,100)
        grp.GetYaxis().SetLimits(0,100)
        grp.GetXaxis().SetTitle("distance [cm]")
        grp.GetYaxis().SetTitle(range[2])
        grp.GetYaxis().SetTitleOffset(1.3)
        grp.GetXaxis().SetTitleOffset(1.3)
        grp.Draw("APE")
        doSpam("Primary-Secondary scintillation",0.13,.95,.60,.95, align=12, textSize=0.033)
